  ...
  sorteddaystoshow.forEach(function (day) {
    ...
    var treatments = _.clone(datastorage[day].treatments);
      ...
      treatments.forEach(function (treatment) {
          // set mills from timestamp if mills is not set, must be an another bug... only workaround here :-(
          if (treatment.timestamp && !treatment.mills) {
              console.log("timestamp available: ", treatment.timestamp);
              treatment.mills = parseInt(treatment.timestamp)
          }
      });

    if (options.order === report_plugins.consts.ORDER_NEWESTONTOP) {
        treatments.sort(function(a,b){
            return b.mills - new Date(a.mills);
        });
    } else {
        treatments.sort(function(a,b){
            return a.mills - new Date(b.mills);
        });
    }
